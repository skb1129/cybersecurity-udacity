...
resource "aws_instance" "project_webserver_a" {
  ...
  sudo mkdir /srv/www
  aws s3 cp s3://"${aws_s3_bucket.server-files-bucket-suryakantbansal.bucket}"/server-files.tar.gz /srv/www/server-files.tar.gz
  aws s3 cp s3://"${aws_s3_bucket.server-files-bucket-suryakantbansal.bucket}"/bootstrap.sh /tmp/bootstrap.sh
